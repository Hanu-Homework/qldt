...
export class UserService {
  ...
  initUser() {
    const PROMISE = this.apiService
      .get(this.configService.getRefreshTokenUrl)
      .toPromise()
      .then((response) => {
  ...
  getMyInfo() {
    return this.apiService
      .get(this.configService.getWhoami)
  ...
  update(id: number, user: UserResponseDTO) {
    return this.apiService.put(
      this.configService.getUpdateUserUrl + "/" + id,
      user
    );
  }
  ...
  delete(id: number) {
    return this.apiService.delete(
      this.configService.getDeleteUrl + "/" + id,
      id
    );
  }
  ...
  isUsernameUnique(username: string) {
    return this.apiService.get(
      this.configService.getIsUsernameUniqueUrl + "/" + username
    );
  }
}
